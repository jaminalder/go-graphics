...
| 40 | An arbitrary region is painted by *covering* it with the existing radial pool, not by writing a second watercolour model | `paint.Wash` is radial — one radius per angle — which describes a blob exactly and a crescent not at all, and that looked like a reason to build an analytic per-pixel watercolour for sketch 009. It was: harder-edged, darker and flatter than the real thing, because the pool in `water.go` is luminous *for a reason* — it is a stack of forty near-transparent deposits, and a closed form does not recover that. The answer is the one a painter uses: there is no crescent-shaped brush, so lay several round touches that between them cover the shape. `paint.Wash.Fill` takes a `Region` — anything that can say how deep inside itself a point lies — and covers it with pools sized to that depth, so a foam cell, a disc and a polygon are all painted by the same model. Two details are load-bearing: a hill climb finds each touch's deepest point, because darts alone never land on it and every pool came out a fraction of its proper size; and a candidate is kept only if its centre lies strictly outside every pool already down, because admitting overlapping touches turns a round cell into a cluster of blobs. |
