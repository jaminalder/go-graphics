...
| 32 | Sketch 008 borrows QQL's *structures*, not its flow field | QQL seeds flow lines from a structure and then traces them; its own comment says the seeding matters more than the field. 008 takes the seeding alone: each arrangement offers candidate positions in a laying order and the spacing rule thins them, which is the same proposes/disposes mechanism with one moving part removed — and it is enough to make orbital, formation and shadows read as three different pieces. Reimplemented in the sketch rather than extracted from `qql`: QQL's versions are entangled with its `frame`, its spec machinery and its tracer, so sharing would have meant retrofitting the port and risking its output for a third caller that does not exist. Extract on the second copy that is *actually the same thing*; this one is not, yet. |
