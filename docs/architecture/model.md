...
Each [component view](https://c4model.com/diagrams/component) decomposes exactly one container. Components group code behind a coherent interface. `studio.Persistent` runs in web; `persistence.Worker` and `renderjob.Supervisor` run in the renderer parent; `renderjob.Child` runs in a disposable process. River executes inside the renderer rather than as a separate broker. Shared packages compile into more than one application and do not imply shared memory. The legacy Manager/socket code is retained for tests, not modeled as an active production component.

PostgreSQL and the application-owned private image bucket are logical data stores inside Singular Seed's container boundary. Managed Hetzner hosting of the bucket belongs in the deployment view; the context view keeps this storage implementation inside the system rather than treating it as an independent artwork business system.
