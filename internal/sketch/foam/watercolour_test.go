...
func TestPigmentCrossesAWetWallAndNotADryOne(t *testing.T) {
	...
	for j := range 200 {
		for i := range 200 {
			...
			h := dry.foam.At(u, v)
			if math.IsInf(h.Wall, 1) || h.Next == h.Cell {
				continue
			}
...
func TestTheWatercolourIsTheSamePictureAtEverySize(t *testing.T) {
	...
	small := render(t, s, 60)
	large := render(t, s, 180)

	worst, sum := 0.0, 0.0
	for y := range 60 {
		for x := range 60 {
			a := lumAt(small, x, y, 1)
			b := lumAt(large, x*3, y*3, 3)
			d := math.Abs(a - b)
			worst = math.Max(worst, d)
			sum += d
		}
	}
	// Per-pixel disagreement at a hard edge is expected — a 3× render
	// resolves a wall the small one straddles — so this is about the mean.
	if mean := sum / 3600; mean > 0.035 {
		t.Errorf("the same seed differs by %.4f on average between 60px and 180px (worst %.4f)", mean, worst)
	}
}
